...
output="$repo_root/platform/xenon/XenonDS-v0.8.0-source.tar.gz"
...
mkdir -p "$staging/XenonDS-v0.8.0"
git -c "safe.directory=$repo_root" -C "$repo_root" archive --format=tar HEAD | \
    tar -xf - -C "$staging/XenonDS-v0.8.0"
mkdir -p "$staging/XenonDS-v0.8.0/third_party/noods"
tar -C "$repo_root/third_party/noods" --exclude=.git -cf - . | \
    tar -xf - -C "$staging/XenonDS-v0.8.0/third_party/noods"
tar -czf "$output" -C "$staging" XenonDS-v0.8.0
